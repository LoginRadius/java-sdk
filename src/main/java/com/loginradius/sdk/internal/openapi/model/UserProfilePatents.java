...
public class UserProfilePatents extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(UserProfilePatents.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!UserProfilePatents.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'UserProfilePatents' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<UserProfilePatents>() {
                @Override
                public void write(JsonWriter out, UserProfilePatents value) throws IOException {
                ...
                @Override
                public UserProfilePatents read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        UserProfilePatents ret = new UserProfilePatents();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for UserProfilePatents: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public UserProfilePatents() {
        super("oneOf", Boolean.FALSE);
    }

    public UserProfilePatents(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return UserProfilePatents.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to UserProfilePatents
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for UserProfilePatents with oneOf schemas: List<Object>, Object. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of UserProfilePatents given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of UserProfilePatents
     * @throws IOException if the JSON string is invalid with respect to UserProfilePatents
     */
    public static UserProfilePatents fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, UserProfilePatents.class);
    }

    /**
     * Convert an instance of UserProfilePatents to an JSON string
     *
     * @return JSON string
     */

...
public class UserProfileVolunteer extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(UserProfileVolunteer.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!UserProfileVolunteer.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'UserProfileVolunteer' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<UserProfileVolunteer>() {
                @Override
                public void write(JsonWriter out, UserProfileVolunteer value) throws IOException {
                ...
                @Override
                public UserProfileVolunteer read(JsonReader in) throws IOException {
                    ...
                    if (match == 1) {
                        UserProfileVolunteer ret = new UserProfileVolunteer();
                        ret.setActualInstance(actualAdapter.fromJsonTree(jsonElement));
                        return ret;
                    }

                    throw new IOException(String.format("Failed deserialization for UserProfileVolunteer: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public UserProfileVolunteer() {
        super("oneOf", Boolean.FALSE);
    }

    public UserProfileVolunteer(Object o) {
        super("oneOf", Boolean.FALSE);
        setActualInstance(o);
    }
    ...
    public Map<String, Class<?>> getSchemas() {
        return UserProfileVolunteer.schemas;
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to UserProfileVolunteer
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format("The JSON string is invalid for UserProfileVolunteer with oneOf schemas: List<Object>, Object. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }

    /**
     * Create an instance of UserProfileVolunteer given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of UserProfileVolunteer
     * @throws IOException if the JSON string is invalid with respect to UserProfileVolunteer
     */
    public static UserProfileVolunteer fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, UserProfileVolunteer.class);
    }

    /**
     * Convert an instance of UserProfileVolunteer to an JSON string
     *
     * @return JSON string
     */

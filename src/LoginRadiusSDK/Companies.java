 ...
 *getCompanies
 *
 * this function used for get company name which is following by user(If proxy is enabled).
 *
 * @param token Authentication token
 * @param secret LoginRadius App API Secret
 ...
 *getCompanies
 *
 * this function used for get company name which is following by user(If proxy is enabled with security).
 *
 * @param token Authentication token
 * @param secret LoginRadius App API Secret
 * @param proxyHost Proxy host
 ...
 */
    ...
    private LoginRadiusCompanyFollow[] ProcessRequest(String token, String secret, Proxy proxy) throws IOException, Exception {
        ...
        if (Utility.isValidGuid(token) && Utility.isValidGuid(secret)) {
            ...
            String url = String.format("http://"+Config.Domain+"/GetCompany/%s/%s",URLEncoder.encode(secret),URLEncoder.encode( token));

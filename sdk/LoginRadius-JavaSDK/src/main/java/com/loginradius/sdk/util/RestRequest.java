...
public class RestRequest {
	...
	public RestResponse get(String serviceUrl, Map<String, String> params) {
		...
		}if(params.containsKey("apisecret")){
		...
		try {
			   ...
			   if(authorization!="") {
				...
				String time =getTime();
			 	...
			 	con.setRequestProperty("digest", "SHA-256="+encode(apiSecret,time+":"+URLEncoder.encode(url, "UTF-8").toLowerCase()));
	...
	public RestResponse post(String serviceUrl, Map<String, String> getParams, String payload) {
		...
		}if(getParams.containsKey("apisecret")){
			   ...
			   if(sott!="") {
				...
				con.setRequestProperty("Authorization", "Bearer "+authorization); 
				 ...
				 if(payload!=null && !payload.equals(emptyPayload)){
					 con.setRequestProperty("digest", "SHA-256="+encode(apiSecret,time+":"+URLEncoder.encode(url, "UTF-8").toLowerCase()+":"+payload));
					}else{
					 con.setRequestProperty("digest", "SHA-256="+encode(apiSecret,time+":"+URLEncoder.encode(url, "UTF-8").toLowerCase()));
				  }
			   }else if (apiSecret!="" && !LoginRadiusSDK.getRequestSigning()) {
